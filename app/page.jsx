...
import { Plus, Minus, Calendar as CalendarIcon, CheckSquare, Utensils, ShoppingCart, Box, Trash2, Wand2, Copy, X, ChevronDown, ChevronRight, Coffee } from "lucide-react";
...
export default function App() {
  ...
  useEffect(() => {
    if (isLoaded) {
      ...
      const dataToSave = { events, tasks, inventory, recipes, mealPlan };
  ...
  }, [events, tasks, inventory, recipes, mealPlan, isLoaded]);
  ...
  if (!isLoaded) {
    return (
      <div className="min-h-screen bg-[#F4F0E6] flex items-center justify-center">
        <div className="border-4 border-black bg-[#FFD500] p-8 shadow-[8px_8px_0_0_#000] text-3xl font-black uppercase animate-pulse">
          Loading Board...
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col xl:flex-row gap-8">
      ...
      <div className="w-full xl:w-1/4 border-4 border-black bg-white shadow-[8px_8px_0_0_#000] flex flex-col h-[85vh]">
        <div className="bg-[#A7F3D0] p-4 border-b-4 border-black flex flex-col justify-between items-center gap-4">
          <h2 className="text-2xl font-black uppercase">Recipes</h2>
          <div className="flex w-full gap-2">
              <button onClick={() => setIsAiModalOpen(true)} className="flex-1 bg-[#FF90E8] text-black border-4 border-black font-black py-2 hover:bg-black hover:text-[#FF90E8] uppercase shadow-[4px_4px_0_0_#000] active:translate-x-1 active:translate-y-1 active:shadow-none flex items-center justify-center gap-1 transition-all text-xs">
                <Wand2 size={16}/> AI Chef
              </button>
              <button onClick={() => setIsCreatorOpen(true)} className="flex-1 bg-white text-black border-4 border-black font-black py-2 hover:bg-black hover:text-white uppercase shadow-[4px_4px_0_0_#000] active:translate-x-1 active:translate-y-1 active:shadow-none flex items-center justify-center gap-1 transition-all text-xs">
                <Plus size={16}/> Manual
              </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-white border-4 border-black shadow-[16px_16px_0_0_#000] w-full max-w-lg p-6 relative transform rotate-1">
            ...
            {selectedRecipeForDetails.note && (
              <div className="border-4 border-black bg-[#A7F3D0] p-4 mb-6 shadow-[4px_4px_0_0_#000]">
            ...
            <h3 className="font-black uppercase mb-3 text-lg border-b-4 border-black inline-block">Ingredients:</h3>
            <div className="border-4 border-black p-4 space-y-3 bg-[#F4F0E6] max-h-[30vh] overflow-y-auto">
      ...
      {isCreatorOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-white border-4 border-black shadow-[16px_16px_0_0_#000] w-full max-w-2xl p-6 relative h-[90vh] flex flex-col">
            <button onClick={() => setIsCreatorOpen(false)} className="absolute top-4 right-4 bg-[#FF8A8A] border-4 border-black w-10 h-10 flex items-center justify-center font-black text-xl shadow-[4px_4px_0_0_#000] hover:bg-black hover:text-white active:translate-x-1 active:translate-y-1 active:shadow-none"><X size={20}/></button>
            ...
            <div className="flex-1 overflow-y-auto pr-2 space-y-4">
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  <label className="block font-black uppercase text-xs mb-1">Time</label>
                ...
                <div className="flex-1">
                  <label className="block font-black uppercase text-xs mb-1">Temp</label>
              ...
              <div className="border-4 border-black bg-[#F4F0E6] p-4 shadow-[4px_4px_0_0_#000]">
                ...
                <div className="mb-4 space-y-2 max-h-[150px] overflow-y-auto border-4 border-dashed border-black p-2">
                  {Object.entries(newRIngredients).map(([iId, qty]) => {
                    ...
                    return (
                      <div key={iId} className="flex justify-between items-center bg-white border-2 border-black p-2 font-bold uppercase text-sm">
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => removeIngredientQty(iId)} className="w-6 h-6 bg-[#FF8A8A] border-2 border-black hover:bg-black hover:text-white flex items-center justify-center">-</button>
                          <span>{qty}</span>
                ...
                <h3 className="font-black uppercase mb-2 text-xs">Tap Stash to Add:</h3>
                <div className="border-t-4 border-black pt-2 max-h-48 overflow-y-auto grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {inventory.map(inv => (
                    <button key={inv.id} onClick={() => toggleIngredient(inv.id)} className="border-2 border-black bg-white p-1 hover:bg-[#FFD500] text-sm font-black uppercase flex items-center gap-1 text-left shadow-[2px_2px_0_0_#000] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none">
